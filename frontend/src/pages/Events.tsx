...
const Events: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const filtered = events.filter((event) => {
      ...
      const matchesTags =
        ...
        selectedTags.every((tag) => event.category.includes(tag));
  ...
  const allTags = [...new Set(events.flatMap((event) => event.category))];
  ...
  const filterByDate = (dateType: "all" | "upcoming" | "past") => {
    ...
    if (dateType === "all") {
      ...
      const past = events.filter((event) => new Date(event.start_time) < today);
          ...
          <div className="bg-white rounded-lg shadow p-6 mb-8">
            <div className="flex flex-col md:flex-row gap-4 mb-6">
              ...
              <Tabs defaultValue="all" className="w-full md:w-auto">
                <TabsList>
                  <TabsTrigger value="all" onClick={() => filterByDate("all")}>
                    All
                  </TabsTrigger>
                  <TabsTrigger
                    value="upcoming"
